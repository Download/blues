...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create blues-add2
  fly apps create blues-add2-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app blues-add2
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app blues-add2-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'blues-add2-staging' does not match app name in config file 'blues-add2'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name blues-add2-db
  fly postgres attach --app blues-add2 blues-add2-db

  fly postgres create --name blues-add2-staging-db
  fly postgres attach --app blues-add2-staging blues-add2-staging-db
  ```
